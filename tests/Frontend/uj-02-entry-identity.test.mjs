...
test('gateway explains action before architecture and offers only real public next steps', async () => {
  ...
  assert.match(content, /route\('login'\)/);
  assert.match(content, /href="#comment-agir"/);
  assert.match(content, /adhésion à une ZUMRA/i);
  assert.doesNotMatch(content, /route\('landing'\)/);
  assert.doesNotMatch(content, /Découvrir le réseau/);
});

test('retired public discovery is a compatibility redirect and is absent from the sitemap', async () => {
  const controller = await read('app/Http/Controllers/LandingController.php');
  const routes = await read('routes/web.php');
  const sitemap = await read('public/sitemap.xml');

  assert.match(routes, /Route::get\('\/decouvrir', LandingController::class\)->name\('landing'\)/);
  assert.match(controller, /redirect\(\)->route\('gateway', status: 301\)/);
  assert.match(controller, /Découvrir.*fonction du réseau/s);
  assert.doesNotMatch(sitemap, /\/decouvrir/);
  assert.match(sitemap, /https:\/\/dgafrique\.com\//);
});
...
// Regression for undefined --color-* variables that previously made public CTAs unreadable.
