{{--
    Carte Transmission du Fil. Une Transmission ne crée jamais un second fil : elle
    s'intègre au Fil global unique, avec les mêmes actions revalidées à la projection
    (fiche TRANSMISSION §19).
--}}
@props(['item'])
<article class="dg-card" style="display:flex;flex-direction:column;gap:14px">
    ...
    <x-dg.actions>
        <x-dg.btn variant="quiet" :href="$item['action_url']">Voir la Transmission</x-dg.btn>
        @if($item['comment_url'])
