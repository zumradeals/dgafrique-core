<x-layouts.admin title="Accompagnement projets — Administration DG Afrique" current="accompaniment">
    ...
    <main class="admin-content"><div class="admin-heading"><div><p class="eyebrow dark">Administration DG Afrique · CAP‑016</p><h1>Piloter l’accompagnement des projets</h1><p>Configurer les formes d’appui disponibles et tracer les interventions réelles. Aucun réglage ici ne transfère la propriété ou le contrôle d’un projet.</p></div><div class="admin-links"><a href="{{ route('administration.projects.edit') }}">Configuration projets</a><a href="{{ route('projects.index') }}">Voir les projets</a><a href="{{ route('member.space') }}">Mon espace</a></div></div>
        ...
        <section class="admin-accompaniment-list"><div class="section-heading"><div><p class="eyebrow dark">CAP‑045</p><h2>Demandes en attente</h2><p>File déterministe, triée par date de demande croissante. Aucun score n’est calculé.</p></div><small>{{ $pendingRequests->count() }} demande(s)</small></div>
            @forelse($pendingRequests as $accompanimentRequest)
                <article class="admin-accompaniment-card"><header><div><span>{{ $accompanimentRequest->accompaniment->project->domain }}</span><h3>{{ $accompanimentRequest->accompaniment->project->name }} — {{ $accompanimentRequest->subject }}</h3><p>Transmise le {{ $accompanimentRequest->requested_at?->format('d/m/Y à H:i') }}</p></div></header>
                    <p>{{ $accompanimentRequest->description }}</p>
                    <form method="POST" action="{{ route('administration.project-accompaniment.requests.acknowledge', $accompanimentRequest) }}" class="accompaniment-action-form">@csrf @method('PUT')<button class="primary-button">Prendre en charge</button></form>
                    <form method="POST" action="{{ route('administration.project-accompaniment.requests.close', $accompanimentRequest) }}" class="accompaniment-action-form">@csrf @method('PUT')<label class="wide">Note de résolution<textarea name="resolution_note" maxlength="1500" placeholder="Facultatif"></textarea></label><button class="primary-button">Clore</button></form>
                </article>
            @empty<div class="accompaniment-empty"><strong>Aucune demande en attente.</strong></div>@endforelse
        </section>
