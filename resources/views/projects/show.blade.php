...
<main class="ps-main" id="vue-ensemble">
    ...
    <div class="ps-center-grid">
        ...
        <div class="ps-stack"><section class="ps-panel" id="jalons"><div class="ps-panel-head"><h2>Avancement par jalons</h2></div>@forelse($project->milestones->take(4) as $milestone)<div class="ps-milestone"><span>{{ $loop->iteration }}. {{ $milestone->title }}</span><b>{{ $milestone->status === 'COMPLETED' ? 'Terminé' : 'À venir' }}</b><i><em style="width:{{ $milestone->status === 'COMPLETED' ? 100 : 0 }}%"></em></i>@if($canDecide && $milestone->status !== 'COMPLETED')<form method="POST" action="{{ route('projects.milestones.complete', [$project, $milestone]) }}" class="ps-milestone__complete">@csrf @method('PUT')<button type="submit" class="ps-action-link">Marquer accompli →</button></form>@endif</div>@empty<p class="ps-muted">Aucun jalon défini. L’équipe peut structurer son chemin sans qu’aucune étape soit inventée.</p>@endforelse</section>
...
<aside class="ps-right">
    <section class="ps-panel"><div class="ps-panel-head"><h2>État du projet</h2></div><span class="ps-state">{{ $statusLabels[$project->status] ?? $project->status }}</span><p>{{ $isArchived ? 'Ce projet est archivé.' : 'Le projet avance selon les décisions explicites de son porteur et de son équipe.' }}</p>@if(in_array($project->status, [\App\Models\Project::STATUS_IN_PROGRESS, \App\Models\Project::STATUS_COMPLETED], true) && ($canDecide || $myTeamMembership?->status === 'ACTIVE'))<a class="ps-outline" href="{{ route('proofs.create', ['origin_type' => 'PROJECT', 'origin_reference' => $project->public_reference]) }}">Publier une preuve →</a>@endif</section>
        ...
        @if($funding)
            ...
            @if($fundingHistory->isNotEmpty())
                ...
                @foreach($fundingHistory->take(8) as $movement)
                    ...
                    <div class="ps-funding-row">
                        <div class="ps-funding-row__top"><strong>{{ $contributorName }}</strong><span class="ps-funding-row__amount">{{ number_format($movement->amount, 0, ',', ' ') }} ZAHAB</span></div>
                        <div class="ps-funding-row__meta">{{ $movement->occurred_at?->diffForHumans() }} · Confirmé</div>
                    </div>
                @endforeach
            @endif
        @else
