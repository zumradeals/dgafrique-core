<x-layouts.portal title="Mon espace — DG Afrique">
    <div class="space-shell doctrine-dashboard">
        <header class="space-header">
            ...
            <div class="space-identity"><span class="space-notification" aria-label="Aucune nouvelle notification"><i></i></span><details class="account-menu"><summary class="member-summary"><span class="avatar">{{ mb_strtoupper(mb_substr($identity->label, 0, 1)) }}</span><span class="member-name">{{ $identity->label }}</span></summary><div><a href="{{ route('member.profile.edit') }}">Mon profil</a>@if($isAdministrator)<a href="{{ route('administration.profile.edit') }}">Administration</a>@endif<form method="POST" action="{{ route('logout') }}">@csrf<button type="submit">Se déconnecter</button></form></div></details></div>
        </header>

        <div class="space-layout">
            <aside class="space-sidebar"><nav aria-label="Navigation Mon espace"><a class="active nav-blue" href="{{ route('member.space') }}">Vue d’ensemble</a><a class="nav-cyan" href="{{ route('member.profile.edit') }}">Mes capacités</a><span class="nav-amber">Mes besoins <small>à venir</small></span><span class="nav-blue">Opportunités <small>à venir</small></span><a class="nav-green" href="{{ route('zumra.index') }}">Mes ZUMRA <small>aperçu</small></a><span class="nav-amber">Mes projets <small>à venir</small></span><span class="nav-gray">Messages <small>à venir</small></span><span class="nav-gray">Paramètres <small>à venir</small></span>@if($isAdministrator)<a class="nav-cyan" href="{{ route('administration.profile.edit') }}">Administration</a>@endif</nav></aside>

            <main class="space-content doctrine-space-content">
                <section class="dashboard-welcome"><div><p class="eyebrow dark">MON ESPACE D’ACTION</p><h1>Bonjour, {{ $greetingName }}</h1><p>Faites connaître vos capacités, exprimez vos besoins et avancez vers les bonnes opportunités.</p></div><div class="dashboard-identity"><span>Identité reconnue par GAMAD Core</span><strong>{{ $identity->reference }}</strong></div></section>
                ...
                <div class="dashboard-workspace">
                    ...
                    <aside class="dashboard-side-column">
                        <section class="member-foundation"><span class="card-kicker">VOTRE FONDATION</span><div><strong>{{ $profileCompletion }}%</strong><span>Profil de capacités</span></div><div><strong>Actif</strong><span>Compte DG Afrique</span></div><div><strong>—</strong><span>Programme ZUMRA</span></div><small>Aucun statut ZUMRA n’est supposé sans contrat attesté.</small></section>
