<x-layouts.portal title="Mes capacités — DG Afrique">
    ...
    <div class="space-shell progressive-profile">
        ...
        <div class="space-layout">
            <aside class="space-sidebar"><nav><a class="nav-blue" href="{{ route('member.space') }}">Vue d’ensemble</a><a class="active nav-cyan" href="{{ route('member.profile.edit') }}">Mes capacités</a><a class="nav-blue" href="{{ route('people.index') }}">Découvrir des personnes</a><a class="nav-cyan" href="{{ route('recommendations.index') }}">Mes recommandations</a><span class="nav-amber">Mes besoins <small>à venir</small></span><span class="nav-blue">Opportunités <small>à venir</small></span><a class="nav-green" href="{{ route('zumra.index') }}">Mes ZUMRA <small>aperçu</small></a><span class="nav-amber">Mes projets <small>à venir</small></span><span class="nav-gray">Paramètres <small>à venir</small></span></nav></aside>
                                ...
                                <section class="profile-stage" data-profile-step="{{ $loop->index }}" @if($loop->index !== $initialStep) hidden @endif>
                                    ...
                                    @switch($sectionKey)
                                        ...
                                        @case('needs')
                                            ...
                                            @break
                                        @case('collaboration')
                                            <div class="profile-grid"><label>{{ $profileConfiguration['field_labels']['participation_mode'] }}<select name="participation_mode"><option value="">Non précisé</option>@foreach($modes as $mode)<option value="{{ $mode['value'] }}" @selected(old('participation_mode', $profile?->participation_mode)===$mode['value'])>{{ $mode['label'] }}</option>@endforeach</select></label><label>{{ $profileConfiguration['field_labels']['collaboration_preferences'] }}<textarea name="collaboration_preferences_text" rows="5" placeholder="Disponible le week-end&#10;Préférence pour les petits groupes">{{ old('collaboration_preferences_text', \App\Application\Profile\ProfileList::toText($profile?->collaboration_preferences)) }}</textarea></label><label>{{ $profileConfiguration['field_labels']['discovery_display_name'] }}<input name="discovery_display_name" maxlength="120" value="{{ old('discovery_display_name', $profile?->discovery_display_name) }}" placeholder="Votre nom, prénom ou nom d’activité"></label><label>{{ $profileConfiguration['field_labels']['discovery_bio'] }}<textarea name="discovery_bio" rows="5" maxlength="600" placeholder="Ce que vous faites et le type de rencontre utile que vous recherchez">{{ old('discovery_bio', $profile?->discovery_bio) }}</textarea></label></div><label class="consent-card"><input type="checkbox" name="orientation_consent" value="1" @checked(old('orientation_consent', $profile?->orientation_consent))><span><strong>{{ $profileConfiguration['orientation_consent_label'] }}</strong><small>{{ $profileConfiguration['orientation_consent_help'] }}</small></span></label><label class="consent-card discovery-consent"><input type="checkbox" name="discovery_consent" value="1" @checked(old('discovery_consent', $profile?->discovery_consent))><span><strong>{{ $profileConfiguration['discovery_consent_label'] }}</strong><small>{{ $profileConfiguration['discovery_consent_help'] }}</small></span></label>
                                            @break
                                    @endswitch
