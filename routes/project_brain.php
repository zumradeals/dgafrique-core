...
Route::middleware(['web', 'core.member'])->group(function (): void {
    ...
    // PROJET-ZUMRA-INVARIANT-001 — l'ancrage ZUMRA reste une décision humaine explicite, jamais gérée par l'IA.
    Route::post('/projets/cerveau/preparation/{intent}/zumra', [ProjectBrainStartController::class, 'selectZumra'])->whereUuid('intent')->middleware('throttle:project-write')->name('projects.brain.start.zumra.select');
    Route::get('/projets/cerveau/preparation/{intent}/zumra/nouvelle', [ProjectBrainStartController::class, 'newZumra'])->whereUuid('intent')->name('projects.brain.start.zumra.create');
    Route::post('/projets/cerveau/preparation/{intent}/zumra/nouvelle', [ProjectBrainStartController::class, 'storeZumra'])->whereUuid('intent')->middleware('throttle:project-write')->name('projects.brain.start.zumra.store');
