...
i=1; while [ "$i" -le 84 ]; do n=$(printf '%03d' "$i"); test "$(grep -c "^## CAP-$n " docs/capacites/CAPABILITY-COVERAGE.md)" -eq 1; test "$(grep -c "^| CAP-$n " docs/capacites/CAPABILITY-INDEX.md)" -eq 1; i=$((i+1)); done
```
