...
Ce document est un **contrat d'implémentation**. Les cinq points laissés ouverts en version CONCEPTION (0.1) ont été tranchés le 2026-08-18 (§16). L'implémentation peut démarrer.
...
## 15. Modèle de données définitif

```text
dg_notification_reads
  id, core_identity_reference,
  source_type (NEED|PROJECT|ZUMRA|MISSION|TRANSMISSION|PROOF),
  source_reference (chaîne stable : "<id_enregistrement>:<statut>" pour un item actionnable,
                     id d'événement brut pour un item FYI — jamais l'ID seul d'un enregistrement
                     mutable, pour éviter qu'une relecture après changement de statut soit
                     faussement marquée « déjà lue »),
  read_at (nullable), dismissed_at (nullable, réservé, non utilisé par l'UI v1),
  timestamps
  -- unique(core_identity_reference, source_type, source_reference)
```

Aucun titre, corps ni copie d'objet métier n'y est stocké (§3) : chaque item reste calculé en direct depuis sa source à chaque chargement, revalidé (§7/§9) avant affichage.

## 16. Décisions métier validées (2026-08-18)

1. **Surface — aucun badge numérique.** Page personnelle `GET /notifications`, lien texte simple « Notifications » dans le menu compte (même emplacement/patron que « Messages » aujourd'hui, sans badge). Mon espace garde sa priorité dominante unique, inchangée ; Notifications ne s'y insère pas.
2. **Deux catégories d'événements éligibles.** ACTIONNABLE = enveloppe directe des buckets déjà calculés par `myMissionsSections()`/`myTransmissionsSections()`/`mySections()` (invitations, à décider, bloquées, à valider, témoignages demandés, à reconnaître) plus les items décidables Besoin/Projet/ZUMRA construits sur `canDecide()`/`isLeader()` déjà existants. FYI = sous-ensemble curé et explicite d'événements Mission/Transmission/Preuve à destinataire clairement déterminable (participation acceptée/refusée, correction demandée, terminée/annulée, blocage, preuve reconnue/contestée) — jamais un événement sans destinataire humain identifiable avec certitude.
3. **Persistance lu/non lu.** Table dédiée unique `dg_notification_reads` (§15), pointeur par chaîne stable plutôt que polymorphe Eloquent, pour rester simple face à sept sources hétérogènes.
4. **Disparition automatique, pas de masquage manuel.** Un item actionnable résolu dans son module d'origine disparaît automatiquement de « À traiter » au prochain calcul — aucune action de masquage n'est requise ni proposée. Les items FYI lus restent visibles dans « Récentes » tant que leur source reste accessible ; ils sont marqués lus automatiquement à la consultation de la page (même mécanique que `MessageParticipant.last_read_at`), sans bouton dédié.
5. **Paramètres centralisés, écran d'administration reporté.** Une configuration centrale unique (`NotificationConfiguration`, stockée via `PortalSetting` — même mécanisme déjà utilisé par `RecommendationConfiguration`, donc déjà administrable en base sans déploiement) regroupe fenêtre de rétroaction, limites d'affichage et bascules FYI par domaine. Aucun écran d'administration dédié en v1 — noté en dette POST-BETA (§14).

## 17. Instruction d'arrêt
