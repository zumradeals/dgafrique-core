...
`tests/Feature/PartnershipTest.php` — 30 cas : proposition avec `CapabilityStatement` réelle, refus d'une Personne sans `capability_statement_id`, `capability_label` contradictoire écrasé par le libellé réel de la `CapabilityStatement`, capacité d'autrui refusée, consentement au rapprochement requis, contexte inaccessible/inexistant refusé, disponibilité en pause bloque une proposition, activation réservée à l'autorité de contexte, transitions invalides refusées, pause/reprise réservées au fournisseur, retrait vs fin de partenariat distincts et audités, Organisation partenaire sans `CapabilityStatement` ni identité Core (capacité déclarée en texte libre, seul fallback restant), Organisation fournisseur exige un membre habilité réel, aucune `CapabilityStatement`/Organisation/Projet/Besoin/ZUMRA/Mission créée automatiquement, isolation entre organisations, absence de fuite dans la liste, visibilité publique/active, absence de mutation en lecture, cycle de vie complet et événements attendus, multi-partenariat, contextes ZUMRA et Besoin.
