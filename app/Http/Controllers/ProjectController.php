...
final class ProjectController
{
    ...
    public function show(
        ...
        NeedService $needs,
        ProjectSignalsEngine $signalsEngine,
        PartnershipService $partnerships,
        MissionContextRegistry $missionContexts,
        MissionVisibilityService $missionVisibility,
        ProjectFundingContributionService $fundingContributions
    ): View {
        /** @var CoreIdentity $identity */
        $identity = $request->attributes->get('dg_identity');
        abort_unless($service->canView($project, $identity->reference), 404);

        // PROJECT-001 — la ZUMRA mère est l'ancrage du Projet (zumra_group_id), indépendamment
        // de owner_type qui décrit seulement la gouvernance. La fiche Projet ne perd donc jamais
        // son monde ZUMRA lorsqu'un Projet est porté individuellement.
        $project->load(['milestones', 'zumraGroup', 'autonomyPathway']);
        ...
        // PROJECT-001 — le cockpit montre uniquement les Missions que cet acteur peut réellement
        // voir. Aucune Mission privée n'est exposée par une simple requête contextuelle.
        $projectMissions = Mission::query()
            ->where('context_type', 'PROJECT')
            ->where('context_reference', $project->public_reference)
            ->whereNull('parent_mission_id')
            ->latest('created_at')
            ->limit(30)
            ->get()
            ->filter(fn (Mission $mission): bool => $missionVisibility->canViewMission($mission, $identity->reference))
            ->take(6)
            ->values();
        ...
        return view('projects.show', compact(
            'identity', 'project', 'group', 'maturityHistory', 'isAdministrator', 'teamMembers',
            'myTeamMembership', 'pendingTeamRequests', 'teamProfiles', 'projectNeeds', 'canProposeNeed',
            'canProposeMission', 'projectMissions', 'maturitySignals', 'accompaniment', 'recentEvents',
            'eventActorProfiles', 'lastActivityAt', 'progressPercentage', 'funding', 'fundingCollected',
            'fundingRemaining', 'fundingHistory', 'fundingContributorProfiles', 'fundingContributionToken'
        ) + [
            'configuration' => $configuration->get(),
            'canDecide' => $canDecide,
            'maturityStages' => ProjectMaturityService::STAGES,
            'projectPartnerships' => $presentedPartnerships,
            'manageableOrganizations' => $manageableOrganizations,
            'manageableOrganizationCapabilities' => $manageableOrganizationCapabilities,
        ]);
    }
