    ...
    public function authorityContextReferences(string $actor): array
    {
        ...
        return Need::query()
            ->where(function ($query) use ($actor, $ledGroupIds): void {
                $query->where(fn ($q) => $q->where('owner_type', Need::OWNER_PERSON)->where('owner_reference', $actor))
                    ->orWhere(fn ($q) => $q->where('owner_type', Need::OWNER_GROUP)->whereIn('owner_reference', $ledGroupIds));
            })
